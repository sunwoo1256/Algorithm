...
std::vector<int> dijkstra(std::vector<std::vector<std::pair<int,int>>> &graph, int start, int end, int n){
...
int main(){
    ...
    std::vector<int> history = dijkstra(graph, start, end, n);
    
    int temp = end;
    std::vector<int> ans(1,end);
    int cnt = 1;
    while(temp!=start){
        ans.push_back(history[temp]);
        cnt++;
        temp = history[temp];
    }
    std::cout<<cnt<<'\n';
